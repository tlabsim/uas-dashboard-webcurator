...
<div class="space-y-6">
    <div class="page-header">
        ...
        <p class="text-sm text-gray-600 mt-1">Create and publish news, notices, events, and announcements</p>
    </div>

    <form method="POST" action="{{ route('dashboard.web_curator.posts.store') }}" 
        ...
        <div class="card">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="post_title" value="{{ html_entity_decode(old('post_title', '')) }}"
                       x-model="postTitle"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    @php
                        // Handle both session arrays and collection objects
                        $categoriesData = session('categories', $categories ?? []);
                        if ($categoriesData instanceof \Illuminate\Support\Collection) {
                ...
                <div>
                    ...
                    <input type="text" name="author" value="{{ html_entity_decode(old('author', '')) }}"
            ...
            <div
                x-data="{ isFeatured: {{ old('is_featured') ? 'true' : 'false' }} }"
            ...
            <div class="mb-4" x-data="tagInputManager({
                initialTags: @js(collect(explode(',', html_entity_decode(old('tags', ''))))->map(fn($tag) => trim($tag))->filter()->values()->all()),
            ...
            <div class="mb-4">
                ...
                <textarea name="post_excerpt" rows="3"
                          class="textarea-base"
                          placeholder="Brief summary of the post...">{{ html_entity_decode(old('post_excerpt', '')) }}</textarea>
                <!-- <p class="help-text">Short summary shown in post listings</p> -->
            </div>

            <div class="mb-4" x-data="featuredImageField(@js(old('featured_image_uri', '')))">
                                ...
                                <template x-if="field.type === 'date'">
                                    <div x-data="{ 
                                        ...
                                        <input type="date" 
                                               ...
                                               :class="{'border-red-500': field.required && !dateValue}">
                                    </div>
                                </template>
        ...
        {{-- Attachments Section --}}
        <div class="card" x-data="attachmentManager()" x-show="selectedCategoryId" x-cloak>
            <div class="card-header flex items-center justify-between">
                ...
                <button type="button" 
                        ...
                        :disabled="files.length >= (categorySchema?.attachment_config?.max_files || 10)"
                        class="btn-base btn-primary gap-2"
                        :class="files.length >= (categorySchema?.attachment_config?.max_files || 10) ? 'opacity-50 cursor-not-allowed' : ''">
            ...
            {{-- Main file input for selecting files --}}
            <input type="file" 
                   x-ref="fileInput"
            ...
            {{-- Hidden file inputs that will actually be submitted --}}
            ...
            <div x-show="files.length === 0" class="py-8 text-center" style="color: var(--text-soft);">
                ...
                <p class="text-sm">No files attached yet</p>
        ...
        <div class="card !p-0 overflow-hidden">
            @include('web_curator::partials.editor-shell', [
                'shellId' => 'post-create-content',
                'fieldName' => 'post_content',
                'initialContent' => old('post_content'),
                'label' => 'Post Content',
                'showHeader' => true,
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" 
                ...
                <span x-show="!isSubmitting">Create Post</span>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Re-initialize when Alpine updates the DOM
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('postForm', () => ({
        postTitle: '{{ old('post_title') }}',
        selectedCategoryId: '{{ old('category_id') }}',
        selectedPostStatus: '{{ old('post_status', 'Draft') }}',
        categorySchema: null,
        metadataValues: @js(old('metadata', [])),
        isSubmitting: false,
        ...
        init() {
            ...
            // Load schema if category is already selected (from old() values)
            if (this.selectedCategoryId) {
        ...
        async syncAndSubmit(event) {
            ...
            const requiredSelects = event.target.querySelectorAll('input[type="hidden"][required]');
            let hasErrors = false;
            let errorMessages = [];
            ...
            requiredSelects.forEach(input => {
                if (!input.value || input.value.trim() === '') {
                    hasErrors = true;
                    // Find the parent custom select and highlight it
            ...
            if (this.categorySchema?.meta_schema?.required_fields) {
                this.categorySchema.meta_schema.required_fields.forEach(field => {
                    ...
                    if (field.required && (!value || value.toString().trim() === '')) {
                        ...
                        // Highlight the field
                        const fieldInput = event.target.querySelector(`[name="metadata[${field.key}]"]`);
                        if (fieldInput) {
            ...
            if (hasErrors) {
                ...
                const firstErrorField = event.target.querySelector('.border-red-500');
                if (firstErrorField) {
            ...
            try {
                ...
                alert(error?.message || 'Failed to prepare the post for submission.');
            }
        }
    }));
    ...
    Alpine.data('attachmentManager', () => ({
        files: [],
        fileInputs: [], // Track DataTransfer objects
        ...
        addFiles(event) {
            ...
            const maxFiles = this.attachmentConfig.max_files || 10;
            const allowedTypes = this.attachmentConfig.allowed_types || [];
            ...
            for (const file of newFiles) {
                ...
                // Create a hidden file input with this file
